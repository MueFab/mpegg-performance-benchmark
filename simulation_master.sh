...
# Aligned -------------------------------------------------------------------
 ...
 for g in "${bam_files[@]}"; do
    ...
    for tool in "${bam_tools[@]}"; do
        ...
        bash "${git_root_dir}"/single_simulation.sh -f "${bam_file}" -g "${ref_file}" -t "$tool" -w "$work_dir" -@ "$num_threads" -r "$result_dir" |& tee "$result_dir/$base_name.$tool.log"
        sleep 1
 ...
 for g in "${fastq_gz_files[@]}"; do
    ...
    if [ ${#arrIN[@]} == "1" ]; then
        for tool in "${fastq_tools[@]}"; do
            ...
            echo "********* Start Simulation **********************"
            bash "${git_root_dir}"/single_simulation.sh -f "${file}" -t "$tool" -w "$work_dir" -@ "$num_threads" -r "$result_dir" |& tee "$result_dir/$base_name.$tool.log"
            sleep 1
            echo "********* Finish Simulation **********************"
        done
    else
        ...
        for tool in "${fastq_tools[@]}"; do
            base_name=$(basename ${file})
            echo "********* Start Simulation **********************"
            bash "${git_root_dir}"/single_simulation.sh -f "${file}" -g "${file2}" -t "$tool" -w "$work_dir" -@ "$num_threads" -r "$result_dir" |& tee "$result_dir/$base_name.$tool.log"
            sleep 1
            echo "********* Finish Simulation **********************"
        done
    fi
